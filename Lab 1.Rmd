...
```{r}
length(x)  # What does length() do? gives size of vector or length 
x
```
```{r}
length(y)
...
Square and Squareroot operations
...
Generate a vector of 50 numbers using the rnorm() function
...
rnorm(50) #seed helps you replicate the same number from RNG 
